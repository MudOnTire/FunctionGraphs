...
//绘制ln
function drawLog(originY, scaleX, scaleY) {
	clearCanvas();
	var canvasWidth = graphCanvas.width;
	drawAxis(canvasWidth, originY);
	canvasContext.beginPath();
	canvasContext.strokeStyle = "#ff8300";
	var startX = canvasWidth * 5;
	for (var x = startX; x < canvasWidth * 10; x++) {
		var d = (x - canvasWidth * 5) / scaleX;
		var y = -1 * Math.log(d) * scaleY + originY;
		if (Math.abs(y - originY) > originY || Math.abs(originY) === Infinity) {
			canvasContext.moveTo(x / 10, y);
		} else {
			canvasContext.lineTo(x / 10, y);
		}
...
//指数函数 2^x
function drawXPower(originY, scaleX, scaleY) {
	clearCanvas();
	var canvasWidth = graphCanvas.width;
	drawAxis(canvasWidth, originY);
	canvasContext.beginPath();
	...
	for (var x = 0; x < canvasWidth * 10; x++) {
		var d = (x - canvasWidth * 5) / scaleX;
		var y = -1 * Math.pow(2, d) * scaleY + originY;
		if (Math.abs(y - originY) > originY) {
			canvasContext.moveTo(x / 10, y);
		} else {
			canvasContext.lineTo(x / 10, y);
